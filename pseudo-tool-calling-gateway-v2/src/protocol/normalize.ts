...
// Loose shape of inbound wire messages; we only need a few fields.
// (Local OpenAiChatMessage is declared at the bottom of the file.)
...
interface OpenAiChatMessage { role?: string; content?: unknown; tool_call_id?: string; name?: string; tool_calls?: Array<{ function?: { name?: string; arguments?: string } }> }
  ...
  if (Array.isArray(b.messages)) {
    for (const m of b.messages as OpenAiChatMessage[]) {
      ...
      if (role === "system" || role === "developer") {
        ...
        // Synthesize a [assistant tool_use …] line so the upstream LLM
        // sees the past call in its flattened context.
        const text = (m.tool_calls as Array<Record<string, any>>)
          .map((tc) => `[assistant tool_use ${tc?.function?.name ?? "?"}]`)
          .join("\n");
        messages.push({ role: "assistant", text });
      } else {
        messages.push({ role: role === "assistant" ? "assistant" : "user", text: contentToText(m.content) });
      }
    }
  }
  return {
